...
find_current_pos() {
  ...
  # Use ENVIRON instead of -v to avoid awk interpreting backslash escapes in target
  pos=$(printf '%s\n' "$list" | TARGET="$current_target" awk -F'\t' 'BEGIN{target=ENVIRON["TARGET"]} $1 == target { print NR - 1; exit }')
  echo "${pos:-1}"
}
...
main() {
  ...
  # Override default_mode if a valid mode argument is provided
  if [[ "$arg_mode" == "session" || "$arg_mode" == "window" || "$arg_mode" == "pane" ]]; then
    default_mode="$arg_mode"
  fi
  ...
  sort_mode=$(get_tmux_option "@chawan-sort" "default")
  export CHAWAN_SORT="$sort_mode"

  # Validate keybindings (same pattern as @chawan-key in chawan.tmux)
  local _bind_key _bind_key_pattern='^[a-zA-Z0-9][-a-zA-Z0-9]*$'
  for _bind_key in "$bind_new" "$bind_delete" "$bind_rename"; do
    if [[ ! "$_bind_key" =~ $_bind_key_pattern ]]; then
      display_message "chawan: invalid key binding: $_bind_key"
      return 1
    fi
  done
  ...
  if [[ "$preview_enabled" == "on" ]]; then
    preview_opts=(
      ...
      --preview-window "${preview_position},border-left,+99999"
      --preview-label ''
